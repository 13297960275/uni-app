<template>
	<view class="num-view">
		...
		<view class="box style3" v-if="model==='6'">
			<text :class="disable?'disable':closeMinus?'disable':''" @click.stop="minus">
				<slot name="tipsLeft"></slot>
			</text>
			<input v-model="num" :style="{'font-size':(size*1)+'rpx'}" type="digit" @input.stop="numInput" @blur.stop="blur" :maxlength="maxlength"
			 :disabled="disable?true:!input" class="num" :class="disable?'disable':''" />
			<text :class="disable?'disable':closeAdd?'disable':''" @click.stop="add">
