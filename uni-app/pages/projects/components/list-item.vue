<template>
	<view class="cell-item" :style="{ border: !borderColor ? 'none' : '', borderColor: borderColor ? borderColor : 'transparent'}">
		<view v-for="(item, index) in dataList" :key="index">
			<view v-if="item.type == 'img'" class="cell-img" :style="{flexDirection: item.direction}">
				<image v-if="item.noImage" :src="item.image || '/static/imgs/tx.png'"></image>
				<text>{{item.value}}</text>
			</view>
			<view>
				<text class="m-cell" :style="{ color: item.fontColor }" v-if="item.type == 'm-cell'">{{item.value}}</text>
			</view>
			<view>
