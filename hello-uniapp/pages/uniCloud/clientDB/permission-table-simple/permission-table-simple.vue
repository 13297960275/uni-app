  ...
  <view class="page">
    <view class="top-view">
      ...
      <uni-segmented-control
        @clickItem="typeIndex = $event.currentIndex"
        :values="types.map(({ text }) => text)"
      ></uni-segmented-control>
    </view>
    ...
    <view
      ...
      v-if="item.exclude !== type"
      :key="index"
    >
      <view class="msg">{{ item.msg }}</view>
      <text style="color: #999999"
        >schema路径：uniCloud/database/permission-test-{{
          index + 1
        }}.schema.json\n</text
      >
      <view class="code">
      ...
      <template v-if="index == 3 && type != 'add'">
        ...
        <button
          ...
          size="mini"
          plain
          @click="myFn({ type, index, where: 'uid == $env.uid' })"
        >
          只{{ typeText }}当前角色创建的数据
        </button>
      </template>
        ...
        <button
          ...
          @click="
            myFn({
              type,
              index,
              where: 'create_time > ' + (Date.now() - 60000),
            })
          "
        >
          只{{ typeText }}1分钟内创建的数据
        </button>
      </template>
      ...
      <button
        ...
        size="mini"
        v-if="index == 6"
        @click="myFn({ type, index, action: 'add_view_count' })"
      >
        带上action
        {{ typeText }}
        <text v-if="type != 'create'">表全部数据</text>
