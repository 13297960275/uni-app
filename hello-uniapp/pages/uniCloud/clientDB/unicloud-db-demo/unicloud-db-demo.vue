<template>
  <view class="root">
    <unicloud-db
      ref="udb"
      v-slot:default="{ data, loading, error, options, pagination, hasMore }"
      :options="options"
      :page-data="pageData"
    ...
    >
      <view class="box">
        <view class="info" v-if="!getone">
          <text
            >查询匹配条数：{{
              getcount ? pagination.count : "未知"
            }}，当前页数据条数：{{ data.length }}。</text
          >
        </view>
        <view class="code">
    ...
    <view class="item">
      <view class="row">
        <text class="title"
          >指定查询结果是否仅返回数组第一条数据（getone）</text
        >
        <switch
          class="switch-getone"
          :checked="getone"
          @change="getone = $event.detail.value"
        />
      </view>
      <text class="msg"
        >默认
        false。在false情况下返回的是数组，即便只有一条结果，也需要[0]的方式获取。在值为
        true
        时，直接返回结果数据，少一层数组，一般用于非列表页，比如详情页</text
      >
    </view>
    <template v-if="!getone">
      <view class="item">
        ...
        <text class="msg"
          ...
          replace
          时则替换当前data数据，常用于PC式交互，列表底部有页码分页按钮，默认值为add</text
        >
      </view>
      <view v-if="pageData == 'replace'" class="item row">
        ...
        <uni-number-box
          class="num-box1"
          :min="1"
          @change="changePageCurrent($event / 1)"
          :value="pageCurrent"
        ></uni-number-box>
      </view>
      <view v-else class="item toLoadMore">
        ...
        <view class="msg">
          在列表的加载下一页场景下，使用ref方式访问组件方法，加载更多数据，每加载成功一次，当前页
          +1
        </view>
        <button
          class="loadMore"
      ...
      <view class="item">
        <text class="msg">
          orderby 格式为 字段名 空格 asc(升序)/desc(降序)，多个字段用 ,
          分割，优先级为字段顺序 \n \n
        </text>
      ...
      <view class="item">
        <view class="row">
          ...
          <switch
            class="switch-getcount"
            :checked="getcount"
            @change="getcount = $event.detail.value"
          />
        </view>
    ...
    <view class="item">
      ...
      />
      <text class="msg"
        >多个字段用 , 分割。不写本属性，即表示查询所有字段。支持用 oldname as
        newname方式对返回字段重命名</text
      >
    </view>
  </view>
</template>
