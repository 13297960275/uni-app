<template>
  <view>
    ...
    <view class="tips">
      <view
        >schema配置详见uniCloud-aliyun/database下的book.schema.json、order.schema.json</view
      >
    </view>
    ...
    <uni-section
      ...
      type="line"
    ></uni-section>
    <button @click="getData('book')" plain type="primary">
      查图书book表的数据
    </button>
    <button @click="getData('order')" plain type="primary">
      查订单order表的数据
    </button>
    ...
    <view class="item">
      ...
      <uni-number-box
        class="num-box1"
        :min="1"
        @change="$event / 1 > 0 ? (pageSize = $event / 1) : ''"
        :value="pageSize"
      ></uni-number-box>
    </view>
    <view class="item">
      ...
      <uni-number-box
        class="num-box2"
        :min="1"
        @change="$event / 1 > 0 ? (pageCurrent = $event / 1) : ''"
        :value="pageCurrent"
      ></uni-number-box>
    </view>
    <button @click="getPageData('order')" plain type="primary">
      分页查图书book表的数据
    </button>

    <uni-section
      title="联表查询"
      subTitle="只需在db schema中，将两个表的关联字段建立映射关系，就可以把2个表当做一个虚拟表来直接查询。"
      type="line"
    ></uni-section>
    ...
    <uni-section
      ...
      type="line"
    ></uni-section>
    <button @click="getBookHasCount" plain type="primary">
      查询结果返回总数
    </button>
    ...
    <uni-section
      ...
      type="line"
    ></uni-section>
    <button @click="getBookTitle" plain type="primary">
      仅查询图书数据的书名
    </button>
    ...
    <uni-section
      ...
      type="line"
    ></uni-section>
    <button @click="getBookAs" plain type="primary">
      获得被设置别名的数据
    </button>
    ...
    <button
      plain
      @click="getOrderOrderBy('quantity asc, create_date desc')"
      type="primary"
    >
      销量相同时，按创建时间降序
    </button>
    ...
    <button @click="addData2TestDb()" plain type="primary">
      <text> 在test表里新增一条 "data=当前时间戳"的记录 </text>
    </button>
    <button @click="addMoreData2TestDb()" plain type="primary">
      <text> 在test表里新增5条 "data=随机数"的记录 </text>
    </button>
