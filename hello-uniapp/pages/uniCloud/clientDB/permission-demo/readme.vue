<template>
  <view class="page">
    <alert-code ref="alertCode"></alert-code>
    <uniNoticeBar
      showIcon="true"
      iconType="info"
    ...
    <view class="uni-title tip">
      ...
      <text
        >\n 演示了： 1.用户可以编辑自己的个人资料。
        2.审核员，审核资料的username字段。 3.审核期间用户不能修改username字段。
      ...
      三、数据的介绍，表字段:
      <text
        >\n username(姓名), state(审核状态), nickname(昵称),
        phone(手机号码)</text
      >
    </view>
    ...
    <view class="tips">
      <text
        >DB
        Schema的permission规则，分为两部分，一边是对操作数据的指定，一边是对角色的指定，规则中对两者进行关联，匹配则校验通过。</text
      >
    </view>
    ...
    <button @click="getFn('uid,username,nickname,state')" plain type="primary">
      读取表全部数据
    </button>
    <view class="uni-title pl10">
      配置规则：
    ...
    <uni-section title="根据数据库中的目标数据记录" type="circle"></uni-section>
    ...
    <button
      @click="updateFn({ nickname: '新昵称' }, 'uid == $env.uid')"
      plain
      type="primary"
    >
      <text>更新nickname="新昵称"\n（仅当前用户为创建者的数据）</text>
    </button>
    <button @click="updateFn({ nickname: '新昵称' })" plain type="primary">
      <text>更新nickname="新昵称"\n（表中全部数据）</text>
    </button>
    ...
    <button @click="updateFn({ state: 1 })" plain type="primary">
      <text>更新 state = 1</text>
    </button>
    <view class="uni-title pl10 uni-common-mt">
    ...
    <view class="uni-title pl10 uni-common-mt">
      ...
      <text
        >含义解释：表示执行该操作需要满足，update的表级权限控制外，还需要满足正在被操作的记录的字段state!=0</text
      >
    </view>
    <button @click="updateFn({ username: '新姓名' })" plain type="primary">
      <text>更新 username:'新姓名' \n(表中全部数据)</text>
    </button>
    <button
      @click="updateFn({ username: '新姓名' }, 'uid == $env.uid')"
      plain
      type="primary"
    >
      <text>更新 username:'新姓名' \n(仅当前角色为创建者的数据)</text>
    ...
    <view class="uni-title pl10 uni-common-mt">
      ...
      <text
        >含义解释：综合表级任何用户可读的条件下，新增了未登录游客不能读取phone字段</text
      >
    </view>
    <button @click="getFn('uid,username,nickname,state')" plain type="primary">
      读不带phone字段的数据
    ...
    <button
      @click="getFn('uid,username,nickname,state,phone')"
      plain
      type="primary"
    >
      读带phone字段的数据
