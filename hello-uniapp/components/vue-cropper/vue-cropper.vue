  ...
  >
    <div class="cropper-box">
      ...
      >
        ...
        <!-- <canvas canvas-id="avatar-canvas" id="avatar-canvas" :src="imgs" alt="cropper-img" ref="cropperImg" ></canvas> -->
      </div>
    </div>
    <div
...
export default {
  ...
  methods: {
    ...
    checkOrientationImage (img, orientation, width, height) {
      console.log(img, orientation, width, height);
      let canvas = document.createElement("canvas");
      // let canvas = uni.createCanvasContext('avatar-canvas', this);
    ...
    checkedImg () {
      ...
      uni.getImageInfo({
        ...
        success: function (data) {
          console.log(data);
          _this.orientation = !data.orientation ? 0 : 1;
          let max = _this.maxImgSize;
          _this.imgs = _this.img = data.path;
          if (!_this.orientation && (data.width < max) & (data.height < max)) {
            _this.imgs = _this.img;
            return;
          }

          if (data.width > max) {
            data.height = (data.height / data.width) * max;
            data.width = max;
          }

          if (data.height > max) {
            data.width = (data.width / data.height) * max;
            data.height = max;
          }

          // _this.checkOrientationImage(data.path, _this.orientation, data.width, data.height);
        },
        fail: function (err) {
          console.log(err);
          this.$emit("imgLoad", "error");
